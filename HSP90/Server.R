...
server <- function(input, output, session) {
  ...
  ########################################################### SERVER 2. Genetic resources ####################################################################  
  
  # Rename columns for display
  Metadata_dataframe_renamed <- Metadata_dataframe
  colnames(Metadata_dataframe_renamed) <- gsub("Assembly.level", "Assembly level", colnames(Metadata_dataframe_renamed))
  colnames(Metadata_dataframe_renamed) <- gsub("Data.Bank", "Data Bank", colnames(Metadata_dataframe_renamed))
  
  output$metadata <- renderDT({
    datatable(
      Metadata_dataframe_renamed,
      options = list(
        pageLength = 10,
        scrollX = TRUE,
        dom = 'lftip',  # Show the filter menu 
        columnDefs = list(
          list(className = 'dt-center', targets = "_all"),
          list(
            targets = which(colnames(Metadata_dataframe_renamed) == "Specie") - 1, 
            render = JS(
  ...
  output$image_metadata <- renderUI({
    ...
    authors <- image_data[[species]]$authors[idx]
    source <- image_data[[species]]$source
    license_url <- image_data[[species]]$license[idx]
    license_name <- get_license_name(license_url)
    
    HTML(
      paste0(
        "<span style='white-space: nowrap;'>",
        "Photograph by <strong>", authors, "</strong>, available on <strong>", source, "</strong>, ",
        "under a <a href='", license_url, "' target='_blank'>", license_name, "</a>.",
        "</span>"
      )
    )
  })
  ...
  ##################################################### SERVER 4. HSP90 FAGACEAE DATABASE  ##########################################################
  
  format_go_terms <- function(text_vector) {
    sapply(text_vector, function(x) {
      if (is.na(x) || x == "") return("")
      terms <- unlist(strsplit(x, "•"))
      terms <- terms[terms != ""]
      paste0("•", terms) |> paste(collapse = "<br>")
    })
  }
  ...
  final_dataframe_renamed <- reactive({
    ...
    # icon for the table
    df$`Protein sequence` <- paste0(
      '<i class="fas fa-dna" style="cursor: pointer; color: #28a745; font-size: 20px;" ',
      'data-protein="', df$Protein, '" title="View protein sequence"></i>'
    )
    
    # protein sequence
    df$Sequence_for_download <- df$Sequence
    
    colnames(df) <- gsub("Conserved.domains", "Conserved domains", colnames(df))
    colnames(df) <- gsub("N..AA", "N. AA", colnames(df))
    colnames(df) <- gsub("Subcellular.location", "Subcellular location", colnames(df))
    colnames(df) <- gsub("GO.IDs", "GO IDs", colnames(df))
    colnames(df) <- gsub("GO.Term", "GO Terms", colnames(df))
  ...
  output$proteinTable <- renderDT({
    ...
    df <- final_dataframe_renamed()[, selected_columns, drop = FALSE]
    
    if ("GO IDs" %in% selected_columns) {
      df[["GO IDs"]] <- format_go_terms(df[["GO IDs"]])
    }
    if ("GO Terms" %in% selected_columns) {
      df[["GO Terms"]] <- format_go_terms(df[["GO Terms"]])
    }
    ...
    if ("Gene" %in% selected_columns) {
      datatable_object <- datatable_object %>%
        formatStyle('Gene', fontStyle = 'italic')
    }
  ...
  output$downloadDATABASE <- downloadHandler(
    ...
    content = function(file) {
      ...
      #put icon int the table
      if ("Protein sequence" %in% selected_columns) {
        data_to_download$`Protein sequence` <- data_to_download$Sequence_for_download
      }
      
      # 
      cols_to_clean <- c("Conserved domains", "GO IDs", "GO Terms")
      for (col in intersect(cols_to_clean, colnames(data_to_download))) {
        data_to_download[[col]] <- sapply(data_to_download[[col]], function(x) {
          if (is.na(x) || x == "") return("")
          #
      ...
      # Exportar limpio
      write.csv(data_to_download[, selected_columns, drop = FALSE], 
                file, row.names = FALSE, fileEncoding = "UTF-8")
    }
  )
  
  # ---------------------------------------------------------------
  # Modal showing FASTA sequence + download button
  # ---------------------------------------------------------------
  observeEvent(input$selected_protein_modal, {
    protein <- input$selected_protein_modal
    seq <- final_dataframe$Sequence[final_dataframe$Protein == protein]
    ...
    showModal(modalDialog(
      ...
      size = "l",
      pre(
        style = "white-space: pre-wrap; word-break: break-all; font-size: 16px; max-height: none; overflow: visible;",
        fasta
      ),
      footer = tagList(
        modalButton("Close"),
  ...
  ########################################### SERVER 5.  HSP90 PROTEINS SEQUENCES ############################################################
  ...
  output$species_info <- renderUI({
    ...
    count <- sum(final_dataframe$Specie == species_name)
    
    HTML(paste0(
      "<div style='margin-bottom: 15px; padding: 10px; background-color: #f8f9fa; border-radius: 5px; border: 1px solid #ced4da;'>",
      "<b>Selected species:</b> <i>", species_name, "</i><br>",
      "<b>Number of sequences:</b> ", count,
      "</div>"
    ))
  })
  ...
  # Function to generate content of all FASTA for a species
  ...
  # Render download button for all sequences
  ...
  ##################################################### SERVER 6.2 i TOL PHYLOGENETIC MENU  ##########################################################
...
6. To download the Subject Sequence, press the button \"Download Subject Sequence\"."
  ...
  run_blast <- eventReactive(input$run_blast, {
    ...
    blastp_executable <- file.path("./bin2/blastp")
